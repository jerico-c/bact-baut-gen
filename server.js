...
// Menggunakan memoryStorage agar kompatibel dengan Vercel/Netlify
    ...
    try {
        ...
        uploadFields.forEach(field => {
            ...
            if (req.files && req.files[placeholderName] && req.files[placeholderName][0]) {
                data[placeholderName] = req.files[placeholderName][0].buffer;
            } else if (req.body[placeholderName] && typeof req.body[placeholderName] === 'string' && req.body[placeholderName].startsWith('data:image')) {
                 data[placeholderName] = Buffer.from(req.body[placeholderName].split(',')[1], 'base64');
            } else {
        ...
        const doc = new Docxtemplater(zip, {
            ...
            modules: [new imageModule({
                centered: false,
                getImage: (tag) => tag, // Langsung meneruskan buffer
                getSize: (imgBuffer, tag) => {
                    if (!imgBuffer) return [150, 150];
                    const dimensions = imageSize(imgBuffer);
                    const pageMaxWidth = 672; // Batas aman lebar halaman
                    let targetWidth;
